...
$stats = array(
	'total' => array('name' => __('Total with a %s Assigned', __('Customer Risk')), 'value' => count($reports), 'color' => 'FFFFFF'),
);
...
{
	if($report['AssessmentCustRisk']['id'])
	{
		$assessment_cust_risk_id = $report['AssessmentCustRisk']['id'];
		if(!isset($stats['AssessmentCustRisk.'.$assessment_cust_risk_id]))
		{
			$stats['AssessmentCustRisk.'.$assessment_cust_risk_id] = array(
				'name' => $report['AssessmentCustRisk']['name'],
				'value' => 0,
				'color' => str_replace('#', '', $report['AssessmentCustRisk']['color_code_hex']),
...
$pie_data = array(array(__('Assessment %s', __('Customer Risk')), __('num %s', __('Reports')) ));
...
echo $this->element('Utilities.object_dashboard_block', array(
	'title' => __('%s by %s', __('Reports'), __('Assessment %s', __('Customer Risk')) ),
