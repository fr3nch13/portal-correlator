...
foreach($results as $resultId => $result)
{
	...
	$td[$resultId]['total'] = count($result['Reports']);
...
if($as_block)
{
	...
	echo $this->element('Utilities.object_dashboard_block', [
		'title' => __('%s - %s grouped by %s', __('Reports'), __('Totals'), $scopeName),
		'subtitle' => $subtitle,
		'description' => __('Excluding items that have a 0 count. Based on %s related by %s', __('Reports'), $scopeName),
		'content' => $content,
...
else
{
	echo $this->element('Utilities.page_index', [
		'page_title' => __('%s - Counts', __('Reports')),
