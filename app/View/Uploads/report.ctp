<?php 
// File: app/View/Uploads/report.ctp

$page_options = array();
if($report['Report']['user_id'] == AuthComponent::user('id') or $is_editor or $is_contributor)
{
	$page_options[] = $this->Html->link(__('Add File'), array('controller' => 'temp_uploads', 'action' => 'add', 'report_id' => $report['Report']['id']));
}
// content
$th = array();
...
*/
	...
	$actions = $this->Html->link(__('Compare'), array('controller' => 'vectors', 'action' => 'compare_report_upload', $this->params['pass'][0], $upload['Upload']['id']));
