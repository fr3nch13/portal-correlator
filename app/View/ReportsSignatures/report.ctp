<?php 
// File: app/View/ReportsSignatures/report.ctp


$page_options = array();
if($report['Report']['user_id'] == AuthComponent::user('id'))
{
	$page_options[] = $this->Html->link(__('Add %s', __('Signatures')), array('controller' => 'signatures', 'action' => 'add', 'report_id' => $this->params['pass'][0]));
}
...
foreach ($reports_signatures as $i => $reports_signature)
{
	...
	if($reports_signature['Report']['user_id'] == AuthComponent::user('id'))
	{
		$active = array(
			$this->Html->link($active, array('action' => 'toggle', 'active', $reports_signature['ReportsSignature']['id'], 'hash' => 'ui-tabs-12'),array('confirm' => 'Are you sure?')), 
			array('class' => 'actions'),
		);
		if($reports_signature['Report']['user_id'] == AuthComponent::user('id'))
			$actions .= $this->Html->link(__('Remove'),array('action' => 'delete', $reports_signature['ReportsSignature']['id'], 'hash' => 'ui-tabs-12'),array('confirm' => 'Are you sure?'));
	}
