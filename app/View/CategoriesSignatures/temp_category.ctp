<?php 
// File: app/View/CategoriesSignatures/temp_category.ctp


$page_options = array();
if($temp_category['TempCategory']['user_id'] == AuthComponent::user('id'))
{
	$page_options[] = $this->Html->link(__('Add %s', __('Signatures')), array('controller' => 'signatures', 'action' => 'add', 'temp_category_id' => $this->params['pass'][0]));
}
...
foreach ($categories_signatures as $i => $categories_signature)
{
	...
	if($categories_signature['TempCategory']['user_id'] == AuthComponent::user('id'))
	{
		$active = array(
			$this->Html->link($active, array('action' => 'toggle', 'active', $categories_signature['CategoriesSignature']['id'], 'hash' => 'ui-tabs-3'),array('confirm' => 'Are you sure?')), 
			array('class' => 'actions'),
		);
		if($categories_signature['TempCategory']['user_id'] == AuthComponent::user('id'))
			$actions .= $this->Html->link(__('Remove'),array('action' => 'delete', $categories_signature['CategoriesSignature']['id'], 'hash' => 'ui-tabs-3'),array('confirm' => 'Are you sure?'));
	}
