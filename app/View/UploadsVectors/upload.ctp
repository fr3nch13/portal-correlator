...
{
	$active = $this->Wrap->yesNo($uploads_vector['UploadsVector']['active']);
	
	$actions = $this->Html->link(__('View'), array('controller' => 'vectors', 'action' => 'view', $uploads_vector['Vector']['id']));
	
	if($uploads_vector['Upload']['user_id'] == AuthComponent::user('id'))
	{
		$active = array(
			$this->Html->link($active, array('action' => 'toggle', 'active', $uploads_vector['UploadsVector']['id']),array('confirm' => 'Are you sure?')), 
			array('class' => 'actions'),
		);
