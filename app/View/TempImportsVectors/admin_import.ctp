...
$th = array(
	...
	'TempVector.dns_auto_lookup' => array('content' => __('DNS Tracking'), 'options' => array('sort' => 'TempVector.dns_auto_lookup')),
	'TempVector.whois_auto_lookup' => array('content' => __('Whois Tracking'), 'options' => array('sort' => 'TempVector.whois_auto_lookup')),
...
foreach ($temp_imports_vectors as $i => $temp_imports_vector)
{
	...
	$dns_auto_lookup = '';
	$whois_auto_lookup = '';
	if($temp_imports_vector['TempVector']['type'] == 'ipaddress')
	{
		$dns_auto_lookup = $this->Wrap->dnsAutoLookupLevel($temp_imports_vector['TempVector']['dns_auto_lookup'], true);
		$whois_auto_lookup = $this->Wrap->whoisAutoLookupLevel($temp_imports_vector['TempVector']['whois_auto_lookup'], true);
	}
	if($temp_imports_vector['TempVector']['type'] == 'hostname')
	{
		$dns_auto_lookup = $this->Wrap->dnsAutoLookupLevel($temp_imports_vector['TempVector']['dns_auto_lookup'], true);
		$whois_auto_lookup = $this->Wrap->whoisAutoLookupLevel($temp_imports_vector['TempVector']['whois_auto_lookup'], true);
	}
